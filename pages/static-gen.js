...
export default function SaticGen({ movies }) {
  const title = 'Static Generation'
  const test = ["Loko", "Motif"]
  return (
    <FrogLayout>
      <FrogMain title={title}>
        {/* <p className="text-bold text-orange-500 mb-6">{process.env.NEXT_PUBLIC_TOKYO}</p> */}
        <div className="infobox">
          Search term: <span className="font-bold">solo</span>.
        </div>
...
export async function getStaticProps() {
  ...
  const url = process.env.NEXT_PUBLIC_MOVIE_API_PREFIX + "/movie/search/solo"
